import pytest
import pandas as pd
from src.data_prep.categorical import lowercase_column, lowercase_string, extract_title 
...
def test_extract_title(lowercased_df: pd.DataFrame):
    result = extract_title(lowercased_df, col='string')
    assert result['title'].tolist() == ['mme', 'ms', 'mr', 'lady', 'major']

def test_extract_title_with_replacement(lowercased_df: pd.DataFrame):
